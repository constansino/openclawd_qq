...
const NumericIdListRegex = /^\s*$|^[0-9,，;；\s\n]+$/;

const IdListStringSchema = z.string().optional().refine((value) => {
  if (value === undefined) return true;
  return NumericIdListRegex.test(value);
}, "仅支持QQ数字及分隔符（逗号/空格/换行）");
...
const KeywordTriggersSchema = z.string().optional().default("");
...
export const QQConfigSchema = z.object({
  ...
  admins: IdListStringSchema.describe("管理员QQ号（字符串）。示例：\"1838552185,123456789\"。支持逗号/空格/换行分隔。"),
  ...
  allowedGroups: IdListStringSchema.describe("允许响应的群号白名单（字符串）。示例：\"883766069 123456789\"。"),
  blockedUsers: IdListStringSchema.describe("用户黑名单QQ号（字符串）。示例：\"342571216\" 或 \"342571216,10002\"。"),
  historyLimit: NumberInputSchema(0).describe("群历史注入条数。默认0（推荐，依赖会话系统）；需强保留原文时可设 3~5。"),
  keywordTriggers: KeywordTriggersSchema.describe("关键词触发（字符串）。示例：\"小助手, 帮我\"；支持逗号/空格/换行分隔。"),
